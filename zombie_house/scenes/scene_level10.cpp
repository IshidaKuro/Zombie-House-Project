...
void Level10Scene::Load() {
  cout << "Scene 10 Load" << endl;
  ls::loadLevelFile("res/levels/level_10.txt", 40.0f);
  ...
  string levelData = l->LoadFile("Level10.dat");
...
void Level10Scene::UnLoad() {
  cout << "Scene 10 UnLoad" << endl;
  player.reset();
  ls::unload();
  Scene::UnLoad();
}

void Level10Scene::Update(const double& dt) {
  Scene::Update(dt);

  const auto pp = player->getPosition();
  if (ls::getTileAt(pp) == ls::END) {
    Engine::ChangeScene(&menu); //as this is the last level, go back to the menu
  } else if (!player->isAlive()) {
    Engine::ChangeScene((Scene*)&level10);
  }
